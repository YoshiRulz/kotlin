 ...
 *  - It is usually not cheap to compute such sets on each query, so their result should be cached properly (see [FirSymbolNamesCache]).
...
abstract class FirSymbolNamesProvider {
    ...
    open fun mayHaveTopLevelCallable(packageFqName: FqName, name: Name): Boolean {
        ...
        // The `packageNamesWithTopLevelCallables` check is implemented in `FirSymbolNamesCache` via `getTopLevelCallableNamesInPackage`. It
        // is probably not worth checking it in uncached situations, since building the set of package names with top-level callables is
        // likely as expensive as just calling an uncached `getTopLevelCallableNamesInPackage`.
