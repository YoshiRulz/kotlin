 ...
 */
abstract class FirSymbolNamesCache(protected val session: FirSession) : FirSymbolNamesProvider() {
...
class FirDelegatingSymbolNamesCache(session: FirSession, private val delegate: FirSymbolNamesProvider) : FirSymbolNamesCache(session) {
...
open class FirCompositeSymbolNamesCache(
    session: FirSession,
    val providers: List<FirSymbolNamesProvider>,
) : FirSymbolNamesCache(session) {
    override fun computeTopLevelClassifierNames(packageFqName: FqName): Set<String>? =
    ...
    companion object {
        fun create(session: FirSession, providers: List<FirSymbolNamesProvider>): FirSymbolNamesProvider = when(providers.size) {
            ...
            1 -> when (val provider = providers.single()) {
                is FirSymbolNamesCache -> provider
                else -> FirDelegatingSymbolNamesCache(session, provider)
            }
            else -> FirCompositeSymbolNamesCache(session, providers)
        }
