...
abstract class AbstractNativeKlibIrTest : AbstractNativeSimpleTest() {
    protected fun runTest(@TestDataFile testPath: String) {
        ...
        val expectedContentsNoSig = File("${testPathFull.canonicalPath.substringBeforeLast(".")}.ir.txt")
        assertIrMatchesExpected(testCompilationResult, expectedContentsNoSig, printSignatures = false)

        val expectedContentsWithSig = File("${testPathFull.canonicalPath.substringBeforeLast(".")}.sig.ir.txt")
    ...
    private fun assertIrMatchesExpected(
        ...
        assertEqualsToFile(
            expectedContents,
            StringUtilRt.convertLineSeparators(klibIr)
        )
    }
