...
private class LLFirCompilerRequiredAnnotationsTargetResolver(
    ...
    private inner class AnnotationTransformer(
        ...
        fun transformAnnotations() {
            annotationMap.values.forEach(fun(annotations: List<FirAnnotation>) {
        ...
        fun calculateDeprecations(target: FirElementWithResolveState) {
            ...
            annotationMap.forEach { (declaration, annotations) ->
                if (declaration is FirProperty || declaration is FirFileAnnotationsContainer) return@forEach
        ...
        fun <T> publishResult(target: T) where T : FirElementWithResolveState, T : FirAnnotationContainer {
            ...
            if (target is FirClassLikeDeclaration) {
                target.replaceDeprecationsProvider(deprecationProvider)
            } else if (target is FirCallableDeclaration) {
                target.replaceDeprecationsProvider(deprecationProvider)
            }
